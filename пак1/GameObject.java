...
import services.community.custom.roulette.model.RouletteComponent;
...
public abstract class GameObject extends EventOwner {
   ...
   private RouletteComponent _rouletteComponent;
   public int inGvG;
   private boolean _registered;
   private boolean _prepare;
   private boolean captureCastleEvent;
   private boolean captureCastleEventOwner;
   public boolean inTreasureHunting;
   public boolean inBossHunting;
   public boolean inRop;
   public Map<Integer, Long> _lastRewPvP;
   public Map<Integer, Long> _lastRewPK;
   private AgathionInstance _agathion;
   private ItemInstance _agathionControlItem;

   protected GameObject() {
      this._reflection = ReflectionManager.DEFAULT;
      this._state = new AtomicInteger(0);
      this._rouletteComponent = null;
      this._registered = false;
      this._prepare = false;
      this.captureCastleEvent = false;
      this.captureCastleEventOwner = false;
      this._lastRewPvP = new ConcurrentSkipListMap();
      this._lastRewPK = new ConcurrentSkipListMap();
      this._agathion = null;
      this._agathionControlItem = null;
   }
   ...
   public boolean isInGvG() {
      Player player = this.getPlayer();
      return player == null ? false : this.inGvG > 0;
   }
   ...
   public void setInGvG(int index) {
      this.inGvG = index;
   }

   public void setReg(boolean f) {
      this._registered = f;
   }

   public boolean isReg() {
      Player player = this.getPlayer();
      return player == null ? false : this._registered;
   }

   public void setPrepare(boolean f) {
      this._prepare = f;
   }

   public boolean isPrepare() {
      Player player = this.getPlayer();
      return player == null ? false : this._prepare;
   }

   public boolean isInCaptureCastleEvent() {
      Player player = this.getPlayer();
      return player == null ? false : this.captureCastleEvent;
   }

   public void setIsInCaptureCastleEvent(boolean value) {
      this.captureCastleEvent = value;
   }

   public boolean isInCaptureCastleEventOwner() {
      Player player = this.getPlayer();
      return player == null ? false : this.captureCastleEventOwner;
   }

   public void setIsInCaptureCastleEventOwner(boolean value) {
      this.captureCastleEventOwner = value;
   }

   public boolean isInTreasureHunting() {
      Player player = this.getPlayer();
      return player == null ? false : this.inTreasureHunting;
   }

   public void setInTreasureHunting(boolean enable) {
      this.inTreasureHunting = enable;
   }

   public boolean isInBossHunting() {
      Player player = this.getPlayer();
      return player == null ? false : this.inBossHunting;
   }

   public void setInBossHunting(boolean enable) {
      this.inBossHunting = enable;
   }

   public boolean isInRop() {
      Player player = this.getPlayer();
      return player == null ? false : this.inRop;
   }

   public void setIsInRop(boolean enable) {
      this.inRop = enable;
   }

   public RouletteComponent getRoulette() {
      return this._rouletteComponent == null ? (this._rouletteComponent = new RouletteComponent(this.getPlayer())) : this._rouletteComponent;
   }
   ...
   public void setAgathion(Player player, AgathionInstance agathion) {
      player.unsetVar("agathion");
      this._agathion = agathion;
      player.autoShot();
      if (agathion == null) {
         if (player.isLogoutStarted() && player.getAgathionControlItem() != null) {
            player.setVar("agathion", String.valueOf(player.getAgathionControlItem().getObjectId()), -1L);
         }

         player.getAgathionControlItem().setCustomFlags(0);
         if (player.getAgathionControlItem() != null) {
            player.setAgathionControlItem((ItemInstance)null);
         }
      }

   }

   public void setAgathionControlItem(Player player, int itemObjId) {
      this.setAgathionControlItem(player.getInventory().getItemByObjectId(itemObjId));
   }

   public void setAgathionControlItem(ItemInstance item) {
      this._agathionControlItem = item;
   }

   public ItemInstance getAgathionControlItem() {
      return this._agathionControlItem;
   }

   public void summonAgathion(Player player) {
      if (this.getAgathion() == null) {
         ItemInstance controlItem = this.getAgathionControlItem();
         if (controlItem != null) {
            Integer npcId = (Integer)ConfigAgathion.AGATHION_DATAS.get(controlItem.getItemId());
            if (npcId != null && npcId != 0) {
               NpcTemplate agathionTemplate = NpcHolder.getInstance().getTemplate(npcId);
               if (agathionTemplate != null) {
                  AgathionInstance agathion = AgathionInstance.restore(controlItem, agathionTemplate, player);
                  if (agathion != null) {
                     this.setAgathion(player, agathion);
                     controlItem.setCustomFlags(63);
                     if (!agathion.isRespawned()) {
                        agathion.setCurrentHp((double)agathion.getMaxHp(), false);
                        agathion.setCurrentMp((double)agathion.getMaxMp());
                        agathion.updateControlItem();
                        agathion.store();
                     }

                     agathion.setTitle("");
                     agathion.setName(" ");
                     agathion.setReflection(this.getReflection());
                     agathion.spawnMe(Location.findPointToStay(this, 50, 70));
                     agathion.setRunning();
                     agathion.setFollowMode(true);
                  }
               }
            }
         }
